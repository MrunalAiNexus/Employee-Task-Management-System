/**
 * Cryptographic Utility for MVP Authentication
 * Enforces requirement: "Passwords must never be stored as plaintext."
 * Uses SHA-256 with unique per-user cryptographic salts.
 */

// Pure JavaScript SHA-256 fallback implementation for guaranteed environment safety
function fallbackSha256(ascii: string): string {
  ...
  for (let idx = 0; idx < 8; idx++) {
    for (let byteIdx = 3; byteIdx >= 0; byteIdx--) {
      const b = (hash[idx] >> (byteIdx * 8)) & 255;
...
/**
 * Compute SHA-256 hash using Web Crypto API when available, with pure JS fallback
 */
export async function sha256(input: string): Promise<string> {
  try {
    ...
    console.warn('Web Crypto unavailable, using fallback SHA-256', err);
  }
  return fallbackSha256(input);
}

/**
 * Hash password with user's specific salt
 */
